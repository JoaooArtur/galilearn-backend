...
namespace Student.UnitTests.Domain.Aggregates;
...
public class StudentTests
{
    ...
    public void Create_ShouldRaiseStudentCreatedEvent()
    {
        ...
        var student = Student.Domain.Aggregates.Student.Create(name, email, password, phone, dateOfBirth);
    ...
    {
        var student = Student.Domain.Aggregates.Student.Create("John Doe", "john@example.com", "password", "123", DateTimeOffset.Now);
    ...
    {
        var student = Student.Domain.Aggregates.Student.Create("Jane", "jane@example.com", "pass", "456", DateTimeOffset.Now);
    ...
    {
        var student = Student.Domain.Aggregates.Student.Create("Alex", "alex@example.com", "pwd", "789", DateTimeOffset.Now);
    ...
    {
        var student = Student.Domain.Aggregates.Student.Create("Anna", "anna@example.com", "pwd", "000", DateTimeOffset.Now);
        var friendId = Guid.NewGuid();
    ...
    {
        var student = Student.Domain.Aggregates.Student.Create("Ana", "ana@example.com", "pwd", "999", DateTimeOffset.Now);
        var friendId = Guid.NewGuid();
        student.CreateFriendRequest(friendId);
        ...
        student.Requests.First().Status.Should().Be(Student.Domain.Enumerations.FriendRequestStatus.Accepted);
    }

    [Fact]
    public void RejectFriendRequest_ShouldChangeRequestStatusToRejected()
    {
        var student = Student.Domain.Aggregates.Student.Create("Leo", "leo@example.com", "pwd", "321", DateTimeOffset.Now);
        var friendId = Guid.NewGuid();
        student.CreateFriendRequest(friendId);
        var requestId = student.Requests.First().Id;
        ...
        student.Requests.First().Status.Should().Be(Student.Domain.Enumerations.FriendRequestStatus.Rejected);
    }

    [Fact]
    public void AddSubjectProgress_ShouldAddSubject()
    {
        var student = Student.Domain.Aggregates.Student.Create("Lucas", "lucas@example.com", "pwd", "888", DateTimeOffset.Now);
        var subjectId = Guid.NewGuid();
    ...
    {
        var student = Student.Domain.Aggregates.Student.Create("Mia", "mia@example.com", "pwd", "777", DateTimeOffset.Now);
        var subjectId = Guid.NewGuid();
        var lessonId = Guid.NewGuid();
        student.AddSubjectProgress(subjectId);
    ...
    {
        var student = Student.Domain.Aggregates.Student.Create("Zoe", "zoe@example.com", "pwd", "333", DateTimeOffset.Now);
